...
class OptimizerSingleIntegrator:
	...
	def __init__(self, initial_state, full_spec, time_bound, time_steps, u_guess, ax):
		...
		u_min = -1
		u_max = 1
		u1_above_min = STLFormula(lambda s, t : s[t,0] - u_min)
		u1_below_max = STLFormula(lambda s, t : -s[t,0] + u_max)
		u2_above_min = STLFormula(lambda s, t : s[t,1] - u_min)
		u2_below_max = STLFormula(lambda s, t : -s[t,1] + u_max)
	...
	def STL_signal(self, u):
		...
		x = copy(self.x0)
		for t in range(T):
			# extract the first and second elements of x
			s[0:2,t] = x[[0,1],:].flatten()
